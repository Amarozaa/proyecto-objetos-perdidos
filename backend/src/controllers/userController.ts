...
export const getMiPerfil = async (
  ...
  try {
    ...
    const usuario = await UsuarioModel.findById(req.userId).select(
      "-passwordHash"
    );
...
export const createUsuario = async (
  ...
  try {
    ...
    const datosUsuario = {
      ...
      telefono: telefonoNormalizado,
      imagen_url: req.file
        ? `/api/images/usuarios/${req.file.filename}`
        : undefined,
    };
  ...
  try {
    ...
    if ("telefono" in datosActualizacion) {
      const telefonoNormalizado =
        datosActualizacion.telefono?.trim() || undefined;
    ...
    if (datosActualizacion.password) {
      const saltRounds = 12;
      datosActualizacion.passwordHash = await bcrypt.hash(
